...
class UserServiceImplTest {
    ...
    void shouldThrowException_whenDefaultUserRoleNotConfigured() {
        when(userRepository.existsByEmail(validRequest.getEmail())).thenReturn(false);
        when(userRepository.existsByPhoneNumber(validRequest.getPhoneNumber())).thenReturn(false);
        when(roleService.findRoleByName(RoleEnum.USER.name())).thenReturn(null);
        ...
        verify(userRepository, times(1)).existsByEmail(validRequest.getEmail());
        verify(userRepository, times(1)).existsByPhoneNumber(validRequest.getPhoneNumber());
        verify(roleService, times(1)).findRoleByName(RoleEnum.USER.name());
    ...
    void shouldRegisterUserSuccessfully() throws Exception {
        when(userRepository.existsByEmail(validRequest.getEmail())).thenReturn(false);
        when(userRepository.existsByPhoneNumber(validRequest.getPhoneNumber())).thenReturn(false);
        when(roleService.findRoleByName(RoleEnum.USER.name())).thenReturn(new Role(1L, "USER", "User role", "ACTIVE", 1L));
        when(passwordEncoder.encode(validRequest.getPassword())).thenReturn("encrypted-password");
        when(verificationCodeService.generateCode(VerificationCodeType.REGISTRATION)).thenReturn("123456");
        ...
        verify(verificationCodeService, times(1)).generateCode(VerificationCodeType.REGISTRATION);
    }
